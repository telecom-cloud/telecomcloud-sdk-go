...
import (
	...
	apiErr "github.com/telecom-cloud/client-go/pkg/openapi/
...
var (
	...
	formUrlEncodeType = "application/x-www-form-urlencoded; charset=utf-8"

	jsonCheck    = regexp.MustCompile(`(?i:(application|text)/(json|.*\+json|json\-.*)(;|$))`)
	xmlCheck     = regexp.MustCompile(`(?i:(application|text)/(xml|.*\+xml)(;|$))`)
	formUrlCheck = regexp.MustCompile(`(?i:(application|text)/(xml|.*\+xml)(;|$))`)
)
...
func defaultRequestBodyBind(c *HttpClient, r *request) (contentType string, body io.Reader, err error) {
	...
	if isJSONType(contentType) &&
		...
		vals := url.Values{}
		err = convertToURLValues(r.bodyParam, &vals)
		if err != nil {
			return
		}
...
func convertToURLValues(bodyParam interface{}, vals *url.Values) error {
	v := reflect.ValueOf(bodyParam)
	switch v.Kind() {
	case reflect.Map:
		for _, key := range v.MapKeys() {
			...
			if value.Kind() == reflect.Slice {
				for i := 0; i < value.Len(); i++ {
					vals.Add(keyStr, value.Index(i).String())
				}
			} else {
				vals.Add(keyStr, value.String())
			}
		}
	case reflect.Struct:
		t := v.Type()
		for i := 0; i < v.NumField(); i++ {
			...
			if value.Kind() == reflect.Slice {
				for i := 0; i < value.Len(); i++ {
					vals.Add(tag, value.Index(i).String())
				}
			} else {
				vals.Add(tag, value.String())
			}
		}
	default:
		return fmt.Errorf("unsupported body type for form encoding: %v", v.Kind())
	}
	return nil
}
