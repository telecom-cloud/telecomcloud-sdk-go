...
func main() {
	...
	createEndpointWhitelist(ctx, client)
}

func createEndpointWhitelist(ctx context.Context, client ve.ClientSet) {
	requestId := utils.GetRandomString(32)
	req := &vpce.CreateEndpointWhitelistRequest{
	...
	resp, raw, err := client.VpcEndpoint().CreateEndpointWhitelist(ctx, req)
	if err != nil {
		fmt.Println(err)
		return
	}
