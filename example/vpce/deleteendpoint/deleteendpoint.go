...
func main() {
	...
	deleteEndpoint(ctx, client)
}

func deleteEndpoint(ctx context.Context, client ve.ClientSet) {
	endpointId := ""
	requestId := utils.GetRandomString(32)
	masterOrderId := ""
	masterResourceId := ""
	req := &vpce.DeleteEndpointRequest{
	...
	resp, raw, err := client.VpcEndpoint().DeleteEndpoint(ctx, req)
	if err != nil {
		fmt.Println(err)
		return
	}
